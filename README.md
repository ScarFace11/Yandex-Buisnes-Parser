...
> **Обновление с 2.3.0 в один клик невозможно.** В той сборке установщик
> записывался файлом с испорченными переводами строк — Windows прерывал его
> до первой замены файла, поэтому приложение закрывалось, а версия оставалась
> прежней. Поставьте 2.3.1 один раз вручную: закройте приложение, распакуйте
> архив релиза в его папку (файлы `.exe` и `_internal` заменятся, данные —
> `.env`, `logs/`, `output/` — останутся на месте). Дальше обновления ставятся
> из приложения.
...
| `build-exe.yml` | тег `v*` | гейт версии, тесты, Windows `.exe` (в него штампуется версия тега), смоук-тест, черновик GitHub Release |
| `build-macos.yml` | тег `v*` | `.app` для arm64 и x64, подпись Developer ID **или ad-hoc**, нотаризация, смоук-тест, `.zip` + `.dmg` |
| `announce-release.yml` | публикация релиза | объявляет версию в `static/version.json` на `main` — только после того, как релиз опубликован и архив на месте |

> Порядок релиза: тег → зелёные билды → **публикация черновика**. Версия
> попадает в `version.json` на последнем шаге, поэтому приложения видят
