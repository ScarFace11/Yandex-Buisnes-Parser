...
# Версия приложения. Суффикс «-dev» делает сборку сборкой РАЗРАБОТЧИКА
# (см. is_dev_build): другой exe, другой порт, без авто-обновления.
# Перед публичным релизом версию выпускают без суффикса: "2.3.0".
APP_VERSION = "2.3.0"


def is_dev_build() -> bool:
    """True для сборки разработчика (версия с суффиксом «-dev»/«-beta»).

    Dev-сборка живёт рядом с публичной и не мешает ей:
      * авто-обновление выключено (публичный релиз не должен затирать
        локальные изменения сборки разработчика);
      * в шапке горит метка DEV, чтобы не перепутать сборки;
      * порт по умолчанию другой (app_port), чтобы обе сборки могли
        работать одновременно.
    Публичный релиз отличается ровно тем, что суффикс убран.
    """
    v = str(APP_VERSION).strip().lower()
    return v.endswith(("-dev", "-beta")) or "-dev." in v or "-beta." in v


# Порт по умолчанию для сборки разработчика — 5010 вместо 5000, чтобы dev- и
# публичная сборки не боролись за один порт.
DEV_PORT = 5010
...
def app_port() -> int:
    """HTTP-порт: переменная YP_PORT → dev 5010 → macOS 5050 → 5000.

    Порядок важен: у dev-сборки свой порт на любой системе, а публичная
